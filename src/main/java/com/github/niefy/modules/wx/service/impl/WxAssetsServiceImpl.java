...
public class WxAssetsServiceImpl implements WxAssetsService {
    ...
    public WxMpMaterialUploadResult materialNewsUpload(String appid, List<WxMpNewsArticle> articles) throws WxErrorException {
        ...
        for (WxMpNewsArticle wxMpNewsArticle : articles) {
            WxMpDraftArticles wxMpDraftArticles = getWxMpDraftArticles(wxMpNewsArticle);
            wxMpDraftArticlesList.add(wxMpDraftArticles);
        }
        wxMpAddDraft.setArticles(wxMpDraftArticlesList);
        String responseContent = wxMpService.getDraftService().addDraft(wxMpAddDraft);
        return WxMpMaterialUploadResult.fromJson(responseContent);
    }

    private static WxMpDraftArticles getWxMpDraftArticles(WxMpNewsArticle wxMpNewsArticle) {
        WxMpDraftArticles wxMpDraftArticles = new WxMpDraftArticles();
        wxMpDraftArticles.setAuthor(wxMpNewsArticle.getAuthor());
        wxMpDraftArticles.setContent(wxMpNewsArticle.getContent());
        wxMpDraftArticles.setDigest(wxMpNewsArticle.getDigest());
        wxMpDraftArticles.setUrl(wxMpNewsArticle.getUrl());
        wxMpDraftArticles.setTitle(wxMpNewsArticle.getTitle());
        wxMpDraftArticles.setThumbMediaId(wxMpNewsArticle.getThumbMediaId());
        wxMpDraftArticles.setContentSourceUrl(wxMpNewsArticle.getContentSourceUrl());
        wxMpDraftArticles.setNeedOpenComment(wxMpNewsArticle.getNeedOpenComment()?1:0);
        wxMpDraftArticles.setOnlyFansCanComment(wxMpNewsArticle.getOnlyFansCanComment()?1:0);

        wxMpDraftArticles.setShowCoverPic(wxMpNewsArticle.isShowCoverPic()?1:0);
